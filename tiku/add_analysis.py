...
"""
为习概题库添加解析的脚本
按照xigai_with_analysis_template.json的模式为所有题目添加解析
"""
...
def generate_single_choice_analysis(question_text, answer, options):
    ...
    if answer and options:
        ...
        if answer_idx < len(options):
            ...
            #         clean_option = re.sub(r'答案：.*', '', clean_option).strip()
            #         analysis += f"选项{option_letter}错误，因为{clean_option}不符合题意。"
...
def extract_keyword(text):
    """从题目中提取关键词"""
    keywords = ["中国特色社会主义", "新时代", "中国梦", "现代化", "党的领导", "改革开放",
                "社会主义", "马克思主义", "习近平新时代中国特色社会主义思想"]
    ...
    # 如果没有匹配的关键词，返回通用描述
    if "什么" in text or "哪" in text:
        return "基本概念"
    elif "为什么" in text or "原因" in text:
        return "原因分析"
    elif "如何" in text or "怎样" in text:
        return "方法途径"
    else:
        return "相关知识"
...
def main():
    # 文件路径
    input_file = "xigai_cleaned.json"
    template_file = "xigai_with_analysis_template.json"
    output_file = "xigai_with_analysis_final.json"

    print("开始处理习概题库...")

    # 加载清理后的题库
    print(f"加载清理后的题库: {input_file}")
