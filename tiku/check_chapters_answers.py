...
def check_question_format(question_text, question_type):
    ...
    # 检查选择题答案格式
    if "选择题" in question_type or "单项" in question_type or "多项" in question_type:
        if answer and not re.match(r'^[A-D]+$', answer):
            issues.append(f"选择题答案格式错误: {answer}")

    # 检查判断题答案格式
    if "判断题" in question_type or "判断" in question_type:
...
def main():
    # 文件路径
    input_file = "chapters_with_analysis.json"
    ...
    # 保存详细报告
    if issues_found > 0:
        report_file = "chapters_answer_check_report.txt"
